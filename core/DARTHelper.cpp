...
MASS::
	MakeBallJointProperties(const std::string &name, const Eigen::Isometry3d &parent_to_joint, const Eigen::Isometry3d &child_to_joint, const Eigen::Vector3d &lower, const Eigen::Vector3d &upper, const double damping, const double friction, const double stiffness)
{
...
MASS::
	MakeRevoluteJointProperties(const std::string &name, const Eigen::Vector3d &axis, const Eigen::Isometry3d &parent_to_joint, const Eigen::Isometry3d &child_to_joint, const Eigen::Vector1d &lower, const Eigen::Vector1d &upper, const double damping)
{
	...
	MakeBodyNode(const SkeletonPtr &skeleton, BodyNode *parent, Joint::Properties *joint_properties, const std::string &joint_type, dart::dynamics::Inertia inertia)
{
	...
	if (joint_type == "Free")
	{
		...
		bn = skeleton->createJointAndBodyNodePair<FreeJoint>(
						 parent, (*prop), BodyNode::AspectProperties(joint_properties->mName))
				 .second;
	}
...
MASS::
	BuildFromFile(const std::string &path, bool create_obj, Eigen::Vector4d color_filter, bool isContact, bool isBVH)
{
	TiXmlDocument doc;
	...
	for (TiXmlElement *node = skeleton_elem->FirstChildElement("Node"); node != nullptr; node = node->NextSiblingElement("Node"))
	{
		...
		if (type == "Free")
		{
			...
			props = MASS::MakeFreeJointProperties(name, parent_to_joint, child_to_joint, damping);
		}
		else if (type == "Planar")
		{
		...
		else if (type == "Revolute")
		{
			...
			props = MASS::MakeRevoluteJointProperties(name, axis, parent_to_joint, child_to_joint, lower, upper, damping);
		}
